...
Connection = this.Connection = Class({
  ...
  _open: function(options){
    ...
    this.db.on('error', function(err) { self._error(new Error(err)); })
    this.db.open(function(err){
      if (err) return self._error(new Error("Can't connect to " + url.format(uri)));
      if (self._close) return self.db.close();
      if(self.uri.username && self.uri.password){
        self.db.authenticate(self.uri.username,self.uri.password,function(){
          self._connect();
        });
      } else self._connect();
    });
...
Collection = this.Collection = Class({
  ...
  setIndexes: function(indexes){
    ...
    indexes.forEach(function(index){
      if (Array.isArray(index)){
        this._ensureIndex(index[0], index[1] && index[1].unique);
  ...
  _ensureIndex: function(obj, unique, fn){
    var indexes = [], callback = fn || empty;
    for (var i in obj){
      indexes.push([i, obj[i]]);
    }
    if (indexes.length) this.ensureIndex(indexes, unique, callback)
  },
